...
plot.mxrpc <- function(x, ...) {
  ...
  # Plot the control chart (customizable via ...)
  default_args <- list(x = 1:m, y = v, type = "b", pch = 20, col = "darkgreen",
                       lwd = 2, ylim = c(LCL * 0.9, UCL * 1.1),
                       xlab = "Sample Number", ylab = y_label,
                       cex.axis = 1.2, cex.main = 1.5)
  do.call(plot, modifyList(default_args, list(...)))
...
plot.mxspc <- function(x, ...) {
  ...
  par(mar = c(5, 5, 4, 10) + 0.1)
  # Plot the control chart (customizable via ...)
  default_args <- list(x = 1:m, y = v, type = "b", pch = 20, col = "darkred",
                       lwd = 2, ylim = c(LCL * 0.9, UCL * 1.1),
                       xlab = "Sample Number", ylab = y_label,
                       cex.axis = 1.2, cex.main = 1.5)
  do.call(plot, modifyList(default_args, list(...)))
